...
class ControlNet(nn.Module):
    ...
    def forward(self, x, hint, timesteps, context, **kwargs):
        ...
        if self.bl_condition_num == 1:
            ...
            hint_2 = hint[:, self.bl_condition_channel[0]:self.bl_condition_channel[0] + self.bl_condition_channel[1],
                     :, :]
            hint_3 = hint[:, self.bl_condition_channel[0] + self.bl_condition_channel[1]:self.bl_condition_channel[0] +
                                                                                         self.bl_condition_channel[1] +
                                                                                         self.bl_condition_channel[2],
                     :, :]
        ...
        # 进行网络前向传播，并在添加 guided_hint 之前调整尺寸
        outs = []
        h = x.type(self.dtype)
...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            ...
            # 拼接并检查控制模型的输入
            concat_input = torch.cat(cond['c_concat'], 1)
            print(f"Concat input min: {concat_input.min()}, max: {concat_input.max()}, mean: {concat_input.mean()}")

            # 运行控制模型
            control = self.control_model(x=x_noisy, hint=concat_input, timesteps=t, context=cond_txt)
            ...
            # 手动替换控制模型输出为随机噪声进行测试
            # control = [torch.randn_like(c) for c in control]  # 取消注释以使用随机噪声代替控制输出

            control = [c * scale for c, scale in zip(control, self.control_scales)]

            # 打印扩散模型输入前的信息
            print(f"Noise input min: {x_noisy.min()}, max: {x_noisy.max()}, mean: {x_noisy.mean()}")
            print(f"Timesteps: {t}")
            print(f"Context min: {cond_txt.min()}, max: {cond_txt.max()}, mean: {cond_txt.mean()}")
        ...
        # 打印扩散模型输出信息
        print(f"Diffusion model output min: {eps.min()}, max: {eps.max()}, mean: {eps.mean()}")
